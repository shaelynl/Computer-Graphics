...
// Define materials.
    ...
    // Dark red (0)
    // change to dark bc light not obvious enough, 
    // since red sphere can only be seen in the reflection
    Material darkRed = Material();
    darkRed.k_d = Color(1.0f, 0.0f, 0.0f);
    darkRed.k_a = darkRed.k_d;
    darkRed.k_r = Color(0.8f, 0.8f, 0.8f) / 1.5f;
    darkRed.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    darkRed.n = 64.0f;
    ...
    purple.k_d = Color(0.8f, 0.4f, 0.8f);
    ...
    black.k_a = Color(0.0f, 0.0f, 0.0f);
    ...
    white.k_a = Color(1.0f, 1.0f, 1.0f);
    ...
    silver.k_d = Color(0.1f, 0.1f, 0.1f);
    silver.k_a = Color(1.0f, 1.0f, 1.0f);
    silver.k_r = Color(0.6f, 0.6f, 0.6f);
    silver.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    silver.n = 2.0f;


// Insert into scene materials vector.

    scene.materials = { darkRed, lightGreen, lightBlue, yellow, purple, black, white, silver };
...
// Draw vertical walls.
    ...
    scene.surfaces[0] = leftVertPlane;
    scene.surfaces[1] = rightVertPlane;

// Draw spheres.

    double radius = 22.0;
    double translate = radius * 2;
    double translate_up = sqrt(pow(radius * 2, 2) - pow(radius, 2)); // Pythagoras' Theorem

    auto bigSphere0 = new Sphere(Vector3d(40.0, 20.0, 40.0), radius, scene.materials[0]);
    auto bigSphere1 = new Sphere(Vector3d(40.0 + translate, 20.0, 40.0), radius, scene.materials[1]);
    auto bigSphere2 = new Sphere(Vector3d(40.0, 20.0, 40.0 + translate), radius, scene.materials[2]);
    auto bigSphere3 = new Sphere(Vector3d(40.0 + translate, 20.0, 40.0 + translate), radius, scene.materials[3]);
    auto bigSphere4 = new Sphere(Vector3d(40.0 + radius, 20.0 + translate_up - 7.0, 40.0 + radius), radius, scene.materials[4]); 
                                                         // not sure why need to offset if not purple sphere will be floating above the rest????

    scene.surfaces[2] = bigSphere0;
    scene.surfaces[3] = bigSphere1;
    scene.surfaces[4] = bigSphere2;
    scene.surfaces[5] = bigSphere3;
    scene.surfaces[6] = bigSphere4;

// Draw horizontal checkered floor.
    ...
    for (int i = 0; i < 11; i++) {
        ...
        for (int j = 0; j < 5; j++) {
            ...
            scene.surfaces[counter++] = new Triangle (Vector3d(0.0 + u, 0.0, 50.0 + v), 
                                                      Vector3d(0.0 + u, 0.0, 0.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 0.0 + v), 
                                                      scene.materials[color0]);

            scene.surfaces[counter++] = new Triangle (Vector3d(0.0 + u, 0.0, 50.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 50.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 0.0 + v), 
                                                      scene.materials[color0]);

            scene.surfaces[counter++] = new Triangle (Vector3d(0.0 + u, 0.0, 100.0 + v),
                                                      Vector3d(0.0 + u, 0.0, 50.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 50.0 + v), 
                                                      scene.materials[color1]);

            scene.surfaces[counter++] = new Triangle (Vector3d(0.0 + u, 0.0, 100.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 100.0 + v),
                                                      Vector3d(50.0 + u, 0.0, 50.0 + v), 
                                                      scene.materials[color1]);
        }
    }
...
// Define camera.
    ...
    scene.camera = Camera (Vector3d(300.0, 240.0, 300.0),       // eye
                           Vector3d(45.0, 22.0, 55.0),          // lookAt
                           Vector3d(0.0, 1.0, 0.0),             // upVector
                           (-1.0 * imageWidth) / imageHeight,   // left
                           (1.0 * imageWidth) / imageHeight,    // right
