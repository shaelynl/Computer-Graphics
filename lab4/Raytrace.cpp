...
// Compute for shadow if hasShadow is true.
    ...
    //***********************************************
    for (const auto& ptlights : scene.ptLights) {
        Vector3d L = ptlights.position - nearestHitRec.p;
        ...
        if (hasShadow) {
            ...
            for (const auto& surface : scene.surfaces) {
                bool hasShadowHit = surface->shadowHit(shadowRay, DEFAULT_TMIN, newTmax);
                if (hasShadowHit) {
                    ks = 0;
                    break;
                }
        ...
        Color phongLighting = ks * computePhongLighting(L, N, V, nearestHitRec.material, ptlights);
        result += phongLighting;
    }
...
// Add to result the reflection of the scene.
    ...
    Vector3d reflectionRayDirection = mirrorReflect(V, N); 
    Ray reflectionRay(nearestHitRec.p, reflectionRayDirection);
    return result + nearestHitRec.material.k_rg * TraceRay(reflectionRay, scene, reflectLevels - 1, hasShadow);
}
