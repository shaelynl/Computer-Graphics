...
// TASK #2: Texture image filenames.
const char whitewoodTexFile[] = "images/whitewood.jpg";
...
// TASK #2: Texture objects.
GLuint whitewoodTexObj;
...
// TASK #2: Forward function declarations.
void DrawCeilingFan(void);
void DrawFanPole(void);
void DrawFanBody(void);
void DrawFanBlades(void);
void RotateFanBlades(void);
...
void MakeReflectionImage( void )
{
    ...
    // STEP 2
    // reflect off the TOP of the table not the middle

    double virtualEyeZ = 2 * TABLETOP_Z - eyePos[2];

    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1], 
              TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0], 
              TABLETOP_Z - virtualEyeZ, TABLETOP_Z - virtualEyeZ + SCENE_RADIUS);
    

    // STEP 3
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(eyePos[0], eyePos[1], virtualEyeZ,
...
// This texture object is for storing the reflection image read from the color buffer.
    ...
    glGenTextures(1, &whitewoodTexObj);
    glBindTexture(GL_TEXTURE_2D, whitewoodTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(whitewoodTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// TASK #2: Added for ceiling fan.
    glutIdleFunc(RotateFanBlades);
...
// Draw a ceiling fan directly above the table
void DrawCeilingFan() {
...
void DrawFanPole() {
    GLfloat matAmbient[] = { 0.7, 0.2, 0.9, 1.0 };
    GLfloat matDiffuse[] = { 0.8, 0.8, 0.8, 1.0 };
    GLfloat matSpecular[] = { 1.0, 1.0, 1.0, 1.0 };
    ...
    GLfloat y = 0.0;
    GLfloat angle = 0.0;
    GLfloat angle_increment = 0.1;

    glTranslated(0.0, 0.0, ROOM_HEIGHT - poleHeight);
    ...
    while (angle < 2 * PI) {
        x = radius * cos(angle);
        y = radius * sin(angle);
        glVertex3f(x, y, poleHeight);
        glVertex3f(x, y, 0.0);
        angle = angle + angle_increment;
    }
    glVertex3f(radius, 0.0, poleHeight);
...
void DrawFanBody() {
    ...
    GLfloat matAmbient[] = { 0.4, 0.8, 0.9, 1.0 };
    GLfloat matDiffuse[] = { 0.8, 0.8, 0.8, 1.0 };
    GLfloat matSpecular[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat matShininess[] = { 64.0 };
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, matAmbient);
    ...
    int i, j;
    int horizontal = 100;       // number of horizontal lines
    int vertical = 100;         // number of vertical lines
    double bodyRadius = 0.2;      

    glTranslated(0.0, 0.0, - bodyRadius);

    for (i = 0; i <= horizontal; i++) {
        double horizontal0 = PI * (-0.5 + (double)(i - 1) / horizontal);
        double z0 = sin(horizontal0);
        double zr0 = cos(horizontal0);

        double horizonatal1 = PI * (-0.5 + (double)i / vertical);
        double z1 = sin(horizonatal1);
        double zr1 = cos(horizonatal1);

        glBegin(GL_QUAD_STRIP);
        for (j = 0; j <= vertical; j++) {
            double vertical0 = 2 * PI * (double)(j - 1) / vertical;
            double x = cos(vertical0);
            double y = sin(vertical0);

            glNormal3f(x * zr0, y * zr0, z0);
            glVertex3f(bodyRadius * x * zr0, bodyRadius * y * zr0, bodyRadius * z0);
            glNormal3f(x * zr1, y * zr1, z1);
            glVertex3f(bodyRadius * x * zr1, bodyRadius * y * zr1, bodyRadius * z1);
        }
        glEnd();
    }
}
...
void DrawFanBlades() {
    ...
    float height = 0;
    float bladeThickness = 0.06;
    float BLADE_X1 = -0.1f;
    float BLADE_X2 = 0.1f;
    float BLADE1_Y1 = 0.2f;
    float BLADE1_Y2 = 1.2f;
    float BLADE2_Y1 = -1.2f;
    float BLADE2_Y2 = -0.2f;


    GLfloat matAmbient[] = { 0.5, 0.3, 0.1, 1.0 };
    GLfloat matDiffuse[] = { 0.8, 0.8, 0.8, 1.0 };
    GLfloat matSpecular[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat matShininess[] = { 64.0 };
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, matAmbient);
    ...
    // RIGHT
    glNormal3f(0.0, 1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X2, BLADE1_Y2, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE1_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X1, BLADE1_Y2, height,
                                0.0, 1.0, BLADE_X2, BLADE1_Y2, height);
    // LEFT
    glNormal3f(0.0, -1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE1_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X2, BLADE1_Y1, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE1_Y1, height,
                                0.0, 1.0, BLADE_X1, BLADE1_Y1, height);
    // FRONT
    glNormal3f(1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X2, BLADE1_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X2, BLADE1_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE1_Y2, height,
                                0.0, 1.0, BLADE_X2, BLADE1_Y1, height);
    // BACK
    glNormal3f(-1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE1_Y2, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE1_Y1, height - bladeThickness,
                                1.0, 1.0, BLADE_X1, BLADE1_Y1, height,
                                0.0, 1.0, BLADE_X1, BLADE1_Y2, height);
    // BOTTOM
    glNormal3f(0.0, 0.0, -1.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE1_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE1_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE1_Y2, height - bladeThickness,
                                0.0, 1.0, BLADE_X2, BLADE1_Y1, height - bladeThickness);
    // TOP
    glNormal3f(0.0, 0.0, 1.0);   // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 1.0, BLADE_X2, BLADE1_Y1, height,
                                1.0, 1.0, BLADE_X2, BLADE1_Y2, height,
                                1.0, 0.0, BLADE_X1, BLADE1_Y2, height,
                                0.0, 0.0, BLADE_X1, BLADE1_Y1, height);

    // BLADE 2

    // RIGHT
    glNormal3f(0.0, 1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X2, BLADE2_Y2, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE2_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X1, BLADE2_Y2, height,
                                0.0, 1.0, BLADE_X2, BLADE2_Y2, height);
    // LEFT
    glNormal3f(0.0, -1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE2_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X2, BLADE2_Y1, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE2_Y1, height,
                                0.0, 1.0, BLADE_X1, BLADE2_Y1, height);
    // FRONT
    glNormal3f(1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X2, BLADE2_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X2, BLADE2_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE2_Y2, height,
                                0.0, 1.0, BLADE_X2, BLADE2_Y1, height);
    // BACK
    glNormal3f(-1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE2_Y2, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE2_Y1, height - bladeThickness,
                                1.0, 1.0, BLADE_X1, BLADE2_Y1, height,
                                0.0, 1.0, BLADE_X1, BLADE2_Y2, height);
    // BOTTOM
    glNormal3f(0.0, 0.0, -1.0); // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 0.0, BLADE_X1, BLADE2_Y1, height - bladeThickness,
                                1.0, 0.0, BLADE_X1, BLADE2_Y2, height - bladeThickness,
                                1.0, 1.0, BLADE_X2, BLADE2_Y2, height - bladeThickness,
                                0.0, 1.0, BLADE_X2, BLADE2_Y1, height - bladeThickness);
    // TOP
    glNormal3f(0.0, 0.0, 1.0);   // Normal vector.
    SubdivideAndDrawQuad(24, 2, 0.0, 1.0, BLADE_X2, BLADE2_Y1, height,
                                1.0, 1.0, BLADE_X2, BLADE2_Y2, height,
                                1.0, 0.0, BLADE_X1, BLADE2_Y2, height,
                                0.0, 0.0, BLADE_X1, BLADE2_Y1, height);
